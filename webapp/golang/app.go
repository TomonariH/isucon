...
var (
	...
	accountNameTmpl *template.Template
	postIDTmpl      *template.Template
	bannedTmpl      *template.Template
)
...
func makePosts(ctx context.Context, results []Post, csrfToken string, allComments bool) ([]Post, error) {
	...
	cntQ, cntArgs, _ := sqlx.In("SELECT `post_id`, COUNT(*) AS `count` FROM `comments` WHERE `post_id` IN (?) GROUP BY `post_id`", postIDs)
	db.SelectContext(ctx, &countRows, cntQ, cntArgs...)
	countMap := make(map[int]int, len(countRows))
	for _, cr := range countRows {
		countMap[cr.PostID] = cr.Count
	}

	// Batch: comments (ORDER BY post_id, created_at DESC → take first 3 per post in Go)
	var allComm []Comment
	commQ, commArgs, _ := sqlx.In("SELECT * FROM `comments` WHERE `post_id` IN (?) ORDER BY `post_id`, `created_at` DESC", postIDs)
	db.SelectContext(ctx, &allComm, commQ, commArgs...)

	commByPost := make(map[int][]Comment, len(postIDs))
	uidSet := make(map[int]struct{})
	for _, c := range allComm {
		if !allComments && len(commByPost[c.PostID]) >= 3 {
			continue
		}
		commByPost[c.PostID] = append(commByPost[c.PostID], c)
		uidSet[c.UserID] = struct{}{}
	}

	// Collect post user IDs
	for _, p := range results {
		uidSet[p.UserID] = struct{}{}
	}
	...
	var users []User
	uQ, uArgs, _ := sqlx.In("SELECT * FROM `users` WHERE `id` IN (?)", uids)
	db.SelectContext(ctx, &users, uQ, uArgs...)
	...
	for _, p := range results {
		...
		comms := make([]Comment, len(commByPost[p.ID]))
		copy(comms, commByPost[p.ID])
		// reverse DESC→ASC
		for i, j := 0, len(comms)-1; i < j; i, j = i+1, j-1 {
			comms[i], comms[j] = comms[j], comms[i]
		}
		for i := range comms {
			comms[i].User = userMap[comms[i].UserID]
		}
		p.Comments = comms
...
func initTemplates() {
	...
	postsTmpl = template.Must(template.New("posts.html").Funcs(fmap).ParseFiles(
		getTemplPath("posts.html"),
		getTemplPath("post.html"),
	))
	accountNameTmpl = template.Must(template.New("layout.html").Funcs(fmap).ParseFiles(
		getTemplPath("layout.html"),
		getTemplPath("user.html"),
		getTemplPath("posts.html"),
		getTemplPath("post.html"),
	))
	postIDTmpl = template.Must(template.New("layout.html").Funcs(fmap).ParseFiles(
		getTemplPath("layout.html"),
...
func getAccountName(w http.ResponseWriter, r *http.Request) {
	...
	postCount := 0
	err = db.GetContext(ctx, &postCount, "SELECT COUNT(*) FROM `posts` WHERE `user_id` = ?", user.ID)
	if err != nil {
		log.Print(err)
		return
	}
	...
	accountNameTmpl.Execute(w, struct {
		Posts          []Post
		User           User
		PostCount      int
		CommentCount   int
...
func getImage(w http.ResponseWriter, r *http.Request) {
	pidStr := r.PathValue("id")
	ext := r.PathValue("ext")
	imgPath := fmt.Sprintf("/home/public/image/%s.%s", pidStr, ext)
	...
	if mimeToExt(post.Mime) != ext {
		w.WriteHeader(http.StatusNotFound)
		return
	}

	os.WriteFile(imgPath, post.Imgdata, 0644)

	w.Header().Set("Content-Type", post.Mime)
	w.Write(post.Imgdata)
}
