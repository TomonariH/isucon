...
no-spec-change 構成では Aurora の parameter group で効く余地が小さい（buffer pool はインスタンスメモリ依存＝spec で封印、durability knob は Aurora で無効、query cache は 8.0 で廃止）。配分はアプリ改修（Phase2: index / N+1 / 不要クエリ削減 / 接続設計 / 既存 reader への参照振り分け）に寄せ、Phase4 は nginx 側（upstream keepalive / 静的配信 / gzip）と app 接続プールなど封印されていない調整に絞る。
