...
1a. **AWS managed 構成の場合はサービス構造を探索する**
   ...
   region が未設定なら、配布情報から `AWS_REGION` を設定する。`/isucon-survey` の内部調査として discovery script を実行する:
   ...
   backend service が確定したら、`/isucon-survey` の内部調査として詳細調査 script を実行する:
   ...
   # SQS ベンチは非同期。送信後この秒数+margin だけ待ってから analyze する（Phase1 で実時間を確定）
   export BENCH_DURATION_SEC='60'
   export BENCH_RESULT_MARGIN_SEC='15'
   ```

   SQS message body の形式は大会ごとに異なるため、自動探索で確定しない場合はユーザーに配布資料、マニュアル URL、スクリーンショットのいずれかを求める。確認できた形式に合わせて `BENCH_MESSAGE_BODY` または `BENCH_MESSAGE_FILE` を設定する。未確認なら `BENCH_MESSAGE_BODY=''` のままにし、`reports/survey.md` に TODO として残す。

   AWS managed 構成では、env を埋めたあと `bash scripts/ecs/iam-check.sh` で analyze/deploy/pprof が依存する AWS API の到達性（OK/DENIED/OTHER）を確認し、DENIED の action と対応する `SKIP_*` を `reports/survey.md` に記録する。score/pass-fail の返却経路・ベンチ実時間（`BENCH_DURATION_SEC`）・nginx LTSV / Aurora slow query or Performance Insights の計測チェーン疎通は、続く Phase 1（`references/ecs/phase1-survey.md` の "Verify IAM Reach" / "Verify Benchmark Completion & Result" / "Verify Measurement Chain"）で確定・通過させる。
...
7. **`scripts/env.sh`・`scripts/alp.yml`・`reports/survey.md` を書き出す**
   ...
   **プレースホルダーはすべて実際の値に置き換えること。**
   ...
   export BENCH_DURATION_SEC='60'
   export BENCH_RESULT_MARGIN_SEC='15'
   # 結果取得経路が確定したら設定（Phase1 / bench-sqs.sh の BENCH_RESULT_MODE）
