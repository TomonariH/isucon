...
if [ -n "$MYSQL_CONF_DIR" ]; then
  ...
  if [ ! -f "$SLOW_CNF" ]; then
    ...
    mkdir -p "$STATE_DIR"
    touch "$STATE_DIR/mysql-slow-config-installed"
  elif ! sudo grep -q 'Generated by setup-mysql.sh' "$SLOW_CNF"; then
    log "WARNING: $SLOW_CNF exists and was not generated by setup-mysql.sh"
  fi
else
...
" 2>&1)"; then
  mkdir -p "$STATE_DIR"
  touch "$STATE_DIR/mysql-slow-enabled"
