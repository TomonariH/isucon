# RDS / Aurora パラメータグループ設定手順

RDS / Aurora では `SET GLOBAL slow_query_log = 1` が使えない場合があるため、パラメータグループで設定する。
Aurora MySQL では DB cluster parameter group と DB instance parameter group が分かれるため、実際に使われている group を両方確認する。
...
2. 使用中の DB parameter group / DB cluster parameter group を選択（またはカスタムグループを新規作成）
3. **パラメータの編集** をクリック
...
export RDS_CLUSTER=isucon-cluster
export RDS_CLUSTER_PARAM_GROUP=isucon-cluster-params

# setup-rds.sh --aws-cli を使う場合
# RDS_PARAM_GROUP / RDS_CLUSTER_PARAM_GROUP の指定された方を変更する
bash scripts/setup-rds.sh --aws-cli

# 手動で DB parameter group を設定する場合
...
# 手動で Aurora DB cluster parameter group を設定する場合
...
cluster parameter group と instance parameter group のどちらに対象 parameter があるかは engine family によって確認する。
