...
0. **前提条件を確認する**
   ...
   **`scripts/env.sh` を Read して環境情報を把握する**
   ...
   確認する変数: `ISUCON_RUNTIME`（docker|systemd）・`ISUCON_WEBAPP_DIR`・`DOCKER_COMPOSE_DIR`・`REBUILD_CMD`
...
3. **アプリをビルドして再起動する**
   ...
   **Docker Compose 環境**: override ファイルも含めて起動する:
   ```bash
   docker compose -f <compose-file> -f docker-compose.pprof.yml up --build -d <app-service>
   ```
