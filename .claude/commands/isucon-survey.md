...
6d. **再ビルドコマンドを導出する**
   ...
   **Docker Compose**:
   ```bash
   docker compose -f <compose-file> build <app-service> && docker compose -f <compose-file> up -d <app-service>
   ```
...
7. **`scripts/env.sh`・`scripts/alp.yml`・`reports/survey.md` を書き出す**
   ...
   **プレースホルダーはすべて実際の値に置き換えること。**
   ...
   パターンが不明な場合は `uri_matching_groups` を空にしておき、初回ベンチ後に追記する。
   ...
   pprof でボトルネック関数を特定できます（ベンチ実行前にセットアップし、ベンチ実行中にプロファイルを取得する）:
