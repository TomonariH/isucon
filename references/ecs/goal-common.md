...
- `BENCH_CMD`
- `BENCH_DURATION_SEC`（SQS ベンチの想定実行時間。既定 60）/ `BENCH_RESULT_MARGIN_SEC`（log 取り込み余裕。既定 15）
- `BENCH_RESULT_MODE`（score/pass-fail 自動取得の将来用 seam。現状未実装で duration-wait にフォールバック）
- `REBUILD_CMD`
- `DB_TYPE=rds|aurora`
...
- `scripts/ecs/analyze.sh` はベンチ窓の CloudWatch メトリクス（Aurora / ALB / Fargate）も `scripts/ecs/metrics.sh` 経由で取得する。アプリ律速・DB 律速・接続律速の切り分けに使い、scale / sizing / DB tuning の判断根拠にする。IAM は `cloudwatch:GetMetricStatistics` が必要。不要なら `SKIP_CW_METRICS=1`。
- `scripts/ecs/pi.sh`（analyze 経由）で Aurora の SQL 別 DB Load（AAS）・wait event を取得し、slow log より正確にボトルネック SQL を順位付けする。IAM は `pi:GetResourceMetrics`。不要なら `SKIP_PI=1`。
...
- SQS ベンチは非同期なので `bench-sqs.sh` は送信後 `BENCH_DURATION_SEC`+margin だけブロックしてから返る。これにより analyze の窓が埋まる。score/pass-fail は Phase1 で返却経路を確定して自動化するまで手動記録。
