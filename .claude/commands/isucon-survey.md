# /isucon-survey

ISUCON 競技開始直後の環境調査を行い、セットアップに必要な情報をまとめてファイルに保存する。

## Steps

0. **課題リポジトリを確認・clone する**

   まず以下を確認する:
   - 競技課題リポジトリの URL はわかっているか？
   - サーバー上に既に課題リポジトリが clone されているか？

   ユーザーに確認してから以下を実行する:
   ```bash
   # clone する場合
   git clone <課題リポジトリ URL> ~/webapp
   # または既にある場合はそのパスを確認
   ls ~/webapp /home/isucon/webapp /opt/isucon 2>/dev/null
   ```

1. **サーバー基本情報を取得する**

   以下のコマンドを実行する:
   ```bash
   uname -m
   grep -E '^(ID|ID_LIKE|VERSION_ID)=' /etc/os-release
   ```

2. **Docker Compose か systemd かを判定し、サービス一覧を取得する**

   まず Docker Compose の有無を確認する:
   ```bash
   ls <webapp-path>/compose.yml <webapp-path>/docker-compose.yml 2>/dev/null
   ```

   **Docker Compose がある場合** — compose.yml からすべての情報を取得する:
   ```bash
   docker compose -f <compose-file> ps
   cat <compose-file>
   ```
   compose.yml から以下を読み取る:
   - サービス名（nginx / app / mysql など）と image/build コンテキスト → 言語を確認
   - `environment:` セクション → DB 接続情報（HOST / PORT / USER / PASSWORD / NAME）
   - `volumes:` セクション → webapp パスとホスト側マウントパス
   - アプリサービスのポート番号

   **systemd の場合** — 以下のコマンドを実行し、Web サーバー・アプリ・DB を特定する:
   ```bash
   systemctl list-units --type=service --state=active --no-pager
   ```
   出力から以下を探す:
   - **Web サーバー**: nginx / h2o / apache2 / httpd
   - **アプリ**: isu* / app* / go* / ruby* / python* / node* / php* / perl* を含むもの
   - **DB（ローカル）**: mysql / mysqld / mariadb / postgresql / redis

3. **アプリの設定から webapp パスと環境変数を取得する**

   **Docker Compose の場合** — Step 2 の compose.yml で取得済み。このステップはスキップ。

   **systemd の場合** — Step 2 で特定したアプリのサービス名を使って実行する:
   ```bash
   sudo systemctl cat <app-service-name>
   ```
   以下を読み取る:
   - `WorkingDirectory=` → webapp のパス
   - `Environment=` → DB 接続情報（HOST / PORT / USER / PASSWORD / NAME）
   - `EnvironmentFile=` → 参照先ファイルのパスを記録し、**必ずそのファイルを `cat` して** 接続情報を取得する
   - `ExecStart=` → 実行バイナリ・言語の確認

4. **アプリのソースコードから DB 接続情報を探す**

   手順2〜3で取得した webapp パスを使って:
   ```bash
   grep -rE '(db_host|DB_HOST|database_host|dsn|DataSourceName|mysql\.open|connectDB)' \
     <webapp-path> --include="*.go" --include="*.rb" --include="*.py" \
     --include="*.js" --include="*.ts" --include="*.php" --include="*.pl" \
     -l 2>/dev/null | head -5
   ```
   見つかったファイルを Read して接続情報（ホスト・ポート・ユーザー・パスワード・DB名）を抽出する。

   `.env` ファイルも確認する:
   ```bash
   find <webapp-path> -maxdepth 3 -name ".env" -o -name "*.env" 2>/dev/null
   ```

5. **DB の種別（ローカル / Docker / RDS / Aurora）を判定する**

   手順2〜4で取得した DB ホスト名を基準に判定する:
   - DB ホストが `*.cluster-*.rds.amazonaws.com` → **Aurora**
   - DB ホストが `*.rds.amazonaws.com` → **RDS**
   - DB ホストが他のプライベート IP → **別サーバーの MySQL**
   - DB ホストが Docker サービス名（compose.yml のサービス名）→ **Docker ローカル MySQL**
   - DB ホストが `localhost` / `127.0.0.1` / 未設定 かつ手順2でローカル DB サービスあり → **systemd ローカル MySQL/MariaDB**

   RDS / Aurora の場合、種別を確認するために以下を実行する:
   ```bash
   mysql -h <db-host> -u <db-user> -p<db-pass> -N -e "SELECT @@version_comment;" 2>/dev/null
   ```

6. **静的ファイルのパスを特定する**

   アプリコードで静的ファイル配信パスを確認する:
   ```bash
   grep -rE '(static|public|assets|StaticFS|ServeFiles|send_file|sendfile)' \
     <webapp-path> --include="*.go" --include="*.rb" --include="*.py" \
     --include="*.js" --include="*.ts" --include="*.php" --include="*.pl" \
     --include="*.psgi" -l 2>/dev/null | head -5
   ```
   見つかったファイルを Read してパスを確認する。

7. **調査結果をファイルに保存し、推奨セットアップコマンドを提示する**

   以下の形式で `reports/survey.md` に Write ツールで保存する。
   Docker / systemd / RDS に応じてセットアップコマンドを切り替える。
   **プレースホルダー（`<host>` など）は調査で取得した実際の値に置き換えること。**

## Output Format

以下の内容を `reports/survey.md` に保存し、チャットにも表示する:

```markdown
# ISUCON 環境サマリー

調査日時: <YYYY-MM-DD HH:MM>

## サーバー
- OS: <distro> <version> (<arch>)
- 実行環境: <Docker Compose | systemd>
- Web サーバー: <nginx|h2o|apache> <version>
- アプリ: <言語> (サービス名: <name>, ポート: <port>)
- webapp パス: <path>
- 静的ファイルパス: <path>

## DB
- 種別: <Docker MySQL|ローカル MySQL|ローカル MariaDB|RDS MySQL|Aurora MySQL|PostgreSQL 等>
- ホスト: <host>
- ポート: <port>
- ユーザー: <user>
- パスワード: <password>
- DB 名: <dbname>

## 推奨セットアップコマンド

\`\`\`bash
# --- Docker Compose 環境の場合 ---
sudo bash scripts/setup-tools.sh
sudo bash scripts/setup-app.sh
DOCKER_COMPOSE_DIR=<compose.ymlのあるディレクトリ> sudo bash scripts/setup-docker.sh

# --- systemd 環境の場合 ---
sudo bash scripts/setup-tools.sh
sudo bash scripts/setup-nginx.sh   # nginx がある場合
sudo bash scripts/setup-mysql.sh   # ローカル MySQL の場合
sudo bash scripts/setup-app.sh

# --- RDS / Aurora の場合 ---
DB_HOST=<host> DB_USER=<user> DB_PASS=<password> sudo bash scripts/setup-rds.sh
\`\`\`
```
