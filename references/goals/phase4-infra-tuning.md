...
- ツールリポジトリで `source scripts/env.sh` して、`TOOL_REPO`・`ISUCON_RUNTIME`・`DOCKER_COMPOSE_DIR`・`DOCKER_COMPOSE_FILE`・`DOCKER_COMPOSE_FILE_ARGS`・`DOCKER_MYSQL_SERVICE`・`NGINX_CONF_HOST_DIR`・`BENCH_CMD`・`REBUILD_CMD` を確認する。
...
2. `$TOOL_REPO/scripts/analyze.sh`（DB が RDS / Aurora なら自動的に `analyze-rds.sh` に委譲される。
   各ベンチでクリーンな slow 窓を得たい場合は `ROTATE_RDS_SLOW_LOG=1 bash $TOOL_REPO/scripts/analyze-rds.sh` を使う）
3. `$TOOL_REPO/scripts/score-log.sh <score> "infra baseline"`
4. MySQL の現状値を確認する。`ISUCON_RUNTIME` / `DB_TYPE` で接続方法を分岐する。
   ...
   systemd + local MySQL もしくは RDS / Aurora（DB へ直接接続して確認する）:
...
3. MySQL connection/cache 系を単体評価する。
   ...
   **RDS / Aurora の場合（候補 1〜3 共通）**: `.cnf` ファイルは配置できない。
   `$TOOL_REPO/templates/rds-parameter-group.md` に従い、パラメータグループで該当値
   （`innodb_buffer_pool_size`、`innodb_flush_log_at_trx_commit`、`sync_binlog`、接続/キャッシュ系）を
   変更する。`ApplyMethod`（`immediate` / `pending-reboot`）と再起動要否を確認してから単体評価する。
   Aurora では `innodb_buffer_pool_size` がインスタンスクラス依存で変更不可な場合がある。
   AWS CLI で変更する場合は `bash $TOOL_REPO/scripts/setup-rds.sh --aws-cli` と同じ要領で
   `aws rds modify-db-parameter-group` を使い、何を変えたか `reports/infra-tuning.md` に記録する。
...
候補 4〜6（nginx / memcached / Redis）は DB が RDS / Aurora でも変わらない。これらはアプリ側サーバーの
nginx・cache 設定なので、`ISUCON_RUNTIME`（systemd / Docker）に応じた通常手順でそのまま適用する。
